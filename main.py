...
def save_feedback(sku_input, name_input, prediction, feedback_type, user_email, correct_product_line=None, correct_business_unit=None, user_comment=None):
    """Save user feedback with enhanced tracking and send Teams notification"""
    try:
        ...
        feedback_data = {
            ...
            'correction_provided': bool(correct_product_line or correct_business_unit),
            'feedback_quality': 'high' if (correct_product_line or correct_business_unit or user_comment) else 'basic'
        }
...
def check_feedback_correction(sku_input, name_input, prediction_product_line=None, prediction_cmr=None):
    ...
    try:
        ...
        retrain_info = load_model_retrain_info()
        if not retrain_info:
        ...
        for feedback in feedback_list:
            ...
            if (sku_match or name_match) and prediction_match and (feedback_type == 'like' or (feedback_type == 'dislike' and has_correction)):
                ...
                if feedback_type == 'like':
                    ...
                    correct_bu = feedback.get('predicted_sub_platform', '')
                else:
                    correct_pl = feedback.get('correct_product_line_code', '')
                    correct_bu = feedback.get('correct_business_unit', '')
                
                return {
                    'feedback_type': feedback_type,
                    'corrected_by': feedback.get('user_name', 'Unknown'),
                    'correction_date': feedback.get('timestamp', ''),
                    'correct_product_line': correct_pl,
                    'correct_business_unit': correct_bu,
                    'user_comment': feedback.get('user_comment', ''),
                    'model_retrain_date': retrain_info.get('last_retrain_date'),
                    'retrained_by': retrain_info.get('retrained_by'),
                    'original_prediction': {
                        'product_line': feedback.get('predicted_product_line_code', ''),
                        'business_unit': feedback.get('predicted_cmr_line', '')
                    }
                }
...
def get_training_data_with_feedback():
    """Convert feedback to training data format with enhanced tracking"""
    ...
    if not positive_feedback.empty:
        likes_data = pd.DataFrame({
            ...
            'feedback_source': 'user_like',
            'feedback_date': positive_feedback['timestamp'],
            'feedback_user': positive_feedback['user_email']
        })
    ...
    if not negative_feedback.empty:
        ...
        if not corrected_feedback.empty:
            ...
            corrections_data = pd.DataFrame({
                ...
                'feedback_date': corrected_feedback['timestamp'],
                'feedback_user': corrected_feedback['user_email']
            })
    ...
    # Combine all training data with metadata
    combined_training_data = pd.concat(training_data_list, ignore_index=True)
    return combined_training_data

def save_model_retrain_info():
    """Save model retraining timestamp and details"""
    try:
        retrain_info = {
            'last_retrain_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'retrained_by': st.session_state.user_email,
            'retrain_count': st.session_state.get('retrain_count', 0) + 1
        }
        
        with open('model_retrain_info.json', 'w') as f:
            json.dump(retrain_info, f, indent=2)
        
        st.session_state.retrain_count = retrain_info['retrain_count']
        st.session_state.last_retrain_date = retrain_info['last_retrain_date']
        
        return retrain_info
    except Exception as e:
        print(f"Error saving retrain info: {str(e)}")
        return None

def load_model_retrain_info():
    """Load model retraining information"""
    try:
        if os.path.exists('model_retrain_info.json'):
            with open('model_retrain_info.json', 'r') as f:
                return json.load(f)
        return None
    except:
        return None
...
def retrain_with_feedback():
    """Retrain model incorporating feedback data with tracking"""
    ...
    if feedback_training_data is not None and not feedback_training_data.empty:
        # Save retraining information
        retrain_info = save_model_retrain_info()
        ...
        if retrain_info:
            st.info(f"🕒 Model last updated: {retrain_info['last_retrain_date']}")
...
def get_exact_predictions(df, sku_partial, name_partial):
    """Find exact substring matches with feedback correction priority"""
    ...
    result_df = df_copy.drop_duplicates(
                ...
                if is_correction:
                    ...
                    if feedback_type == 'like':
                        ...
                        <div style="background-color: #e8f5e8; padding: 10px; border-radius: 5px; border-left: 4px solid #28a745;">
                            <strong>🌟 ENHANCED PREDICTION</strong><br>
                        ...
                        st.markdown("""
                        <div style="background-color: #e8f5e8; padding: 10px; border-radius: 5px; border-left: 4px solid #28a745;">
                            <strong>🔄 RETRAINED PREDICTION</strong><br>
            ...
            if is_correction:
                ...
                if feedback_type == 'like':
                    ...
                    <div style="background-color: #d4edda; padding: 15px; border-radius: 8px; border: 1px solid #c3e6cb;">
                        <h4 style="color: #155724; margin: 0 0 10px 0;">🌟 Enhanced Prediction - User Confirmed</h4>
                        <p style="margin: 5px 0; color: #155724;">This prediction was confirmed as correct by user feedback and reinforced through model retraining.</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown("""
                    <div style="background-color: #fff3cd; padding: 15px; border-radius: 8px; border: 1px solid #ffeaa7;">
                        <h4 style="color: #856404; margin: 0 0 10px 0;">🎆 Enhanced Prediction After Model Retraining</h4>
                        <p style="margin: 5px 0; color: #856404;">This prediction has been improved through machine learning model retraining based on user feedback corrections.</p>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                <div style="background-color: #d1ecf1; padding: 8px; border-radius: 4px; margin-top: 10px;">
                    <small style="color: #0c5460;">ℹ️ This prediction reflects the latest model improvements and should be more accurate than previous versions.</small>
                </div>
                """, unsafe_allow_html=True)
            
            # Feedback buttons
            if is_correction:
                st.markdown("**Is this updated prediction correct?**")
                st.caption("This prediction was already corrected by user feedback")
            else:
...
def display_fuzzy_matches(fuzzy_matches, sku_input, name_input, df_rules):
    ...
    for i, match in enumerate(fuzzy_matches, 1):
        ...
        # Special handling for feedback corrections
        if match.get('is_feedback_correction', False):
            feedback_type = match.get('feedback_type', 'correction')
            if feedback_type == 'like':
                title = f"🌟 ENHANCED Prediction {i}: {match['product_line_code']} - {match['cmr_product_line']} | "
                title += f"{match['product_line_name']} | {match['sub_platform']} "
                title += f"(Confidence: {match['combined_score']}%) - ✅ User Confirmed"
                confidence_color = "🌟"
            else:
                title = f"🔄 RETRAINED Prediction {i}: {match['product_line_code']} - {match['cmr_product_line']} | "
                title += f"{match['product_line_name']} | {match['sub_platform']} "
                title += f"(Confidence: {match['combined_score']}%) - 🔧 User Corrected"
                confidence_color = "🔄"
        else:
            title = f"{confidence_color} Prediction {i}: {match['product_line_code']} - {match['cmr_product_line']} | "
            title += f"{match['product_line_name']} | {match['sub_platform']} "
            title += f"(Confidence: {match['combined_score']}%)"
        ...
        with st.expander(title, expanded=(i == 1)):
            ...
            with col3:
                # Confidence score is now handled in the correction check above
                pass
                
                # Show feedback correction info with enhanced styling
                if match.get('is_feedback_correction', False):
                    st.markdown("""
                    <div style="background-color: #e8f5e8; padding: 15px; border-radius: 8px; border-left: 5px solid #28a745; margin: 10px 0;">
                        <h4 style="color: #155724; margin: 0 0 8px 0;">🔄 RETRAINED PREDICTION</h4>
                        <p style="color: #155724; margin: 0; font-size: 14px;">This prediction was improved through model retraining</p>
                    </div>
                    """, unsafe_allow_html=True)
                    st.caption(f"Originally corrected by: {match.get('corrected_by', 'Unknown')}")
                    st.caption(f"Correction date: {match.get('correction_date', 'Unknown')}")
                    if match.get('model_retrain_date'):
                        st.caption(f"🕒 Model last updated: {match.get('model_retrain_date')}")
                else:
            ...
            if match.get('is_feedback_correction', False):
                ...
                if match.get('correction_comment'):
                    st.write("**Original User Feedback:**", match['correction_comment'])
                
                # Model update information
                col_a, col_b = st.columns(2)
                with col_a:
                    st.write(f"👥 **Originally corrected by:** {match.get('corrected_by', 'Unknown user')}")
                    st.write(f"📅 **Correction date:** {match.get('correction_date', 'Unknown')}")
                with col_b:
                    if match.get('model_retrain_date'):
                        st.write(f"🔄 **Model retrained on:** {match.get('model_retrain_date')}")
                        if match.get('retrained_by'):
                            st.write(f"🔧 **Retrained by:** {match.get('retrained_by')}")
            ...
            # Business Rule Section (skip for feedback corrections)
            if not match.get('is_feedback_correction', False):
                st.markdown("---")
            ...
            # Feedback buttons (different for corrected predictions)
            st.markdown("---")
            if match.get('is_feedback_correction', False):
                st.markdown("**Is this updated prediction correct?**")
                st.caption("This prediction was already corrected by user feedback")
...
def analytics_dashboard():
    """Enhanced analytics dashboard with correction tracking"""
    ...
    # Correction tracking metrics
    if 'correct_product_line_code' in feedback_df.columns:
        st.subheader("🔧 Correction Tracking")
        col1, col2, col3 = st.columns(3)
        ...
        with_comments = len(feedback_df[
            (feedback_df['user_comment'].notna()) & (feedback_df['user_comment'] != '')
        ])
        
        training_ready = likes + corrected_dislikes
        
        with col1:
            st.metric("Corrected Dislikes", corrected_dislikes)
        with col2:
            st.metric("With Comments", with_comments)
        with col3:
            st.metric("Training Ready", training_ready)
            st.caption("Likes + Corrected Dislikes")
    ...
    # Recent feedback with corrections
    st.subheader("Recent Feedback (Last 10)")
    display_cols = ['timestamp', 'user_email', 'sku_input', 'predicted_product_line_code', 'confidence_score', 'feedback']
    if 'correct_product_line_code' in feedback_df.columns:
        display_cols.extend(['correct_product_line_code', 'user_comment'])
    
    recent_feedback = feedback_df.tail(10)[display_cols]
    st.dataframe(recent_feedback, use_container_width=True)
    
    # Show correction examples for model improvement
    if 'correct_product_line_code' in feedback_df.columns:
        corrections = feedback_df[
            (feedback_df['feedback'] == 'dislike') & 
            (feedback_df['correct_product_line_code'].notna()) & 
            (feedback_df['correct_product_line_code'] != '')
        ]
        
        if not corrections.empty:
            st.subheader("📝 Model Improvement Examples")
            st.caption("These corrections help improve future predictions:")
            
            for idx, row in corrections.tail(3).iterrows():
                with st.expander(f"Correction by {row['user_email']} on {row['timestamp']}"):
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write("**Input:**", f"{row['sku_input']} - {row['name_input']}")
                        st.write("**Predicted:**", row['predicted_product_line_code'])
                    with col2:
                        st.write("**Corrected to:**", row['correct_product_line_code'])
                        if row.get('user_comment'):
                            st.write("**Comment:**", row['user_comment'])
...
def main():
    ...
    with col2:
        ...
        <div style="margin-top: -10px;">
            <h1 style='color:#D70000; margin-bottom: 5px;'>🤖 SKU Classification System</h1>
            <h4 style='color:#444; margin-top: 0px;'>Welcome to the Thermo Fisher SKU AI Classifier!</h4>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"<h3 style='color:#D70000;'>Welcome, {st.session_state.user_name}</h3>", unsafe_allow_html=True)
    with col2:
    ...
    with tab3:
        ...
        # Admin-only model retraining section
        ...
        if st.session_state.user_email in admin_users:
            ...
            st.info("⚠️ Admin-only section: Model retraining functionality")
            
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🔄 Retrain Model with Feedback", use_container_width=True, help="Incorporate user feedback into model training"):
            ...
            with col2:
                ...
                if not feedback_df.empty:
                    ...
                    st.metric("Available for Training", f"{likes_count + corrected_dislikes} samples")
                    st.caption(f"👍 {likes_count} likes + 👎 {corrected_dislikes} corrected dislikes")
